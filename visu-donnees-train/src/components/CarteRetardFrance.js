    ...
    Promise.all([
      ...
      .then(([csvData, geoData]) => {
        const trainData = csvData.map((d) => ({
          ...
          nbTrainsRetardes: +d["Nombre de trains en retard à l'arrivée"] || 0,
        }));
        ...
        const colorScale = d3
          .scaleSequential()
          .domain([0, d3.max(trainData, (d) => d.nbTrainsRetardes)])
            ...
            tooltip
              ...
              .html(
                `<strong>Region:</strong> ${regionName}<br>
                 <strong>Train delays:</strong> ${value}`
              );
          })
          .on("mousemove", (event) => {
            tooltip
              .style("top", `${event.pageY - (710+170)}px`)
